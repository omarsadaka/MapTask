...
const RenderForm=({
  ...
  setFieldValue
    ...
    return(
      ...
      <DefaultText title={errors.latitude} style={styles.errorText}/>
      }
       <DefaultInput
       hint={t('app:lon')}
       value={values.logitude.toString()}
       keyboardType={'phone-pad'}
       isLeftIcon={false}
       isRightIcon={true}
       editable={true}
       onChange={handleChange('logitude')}
      />
      {(errors.logitude && touched.logitude) &&
      <DefaultText title={errors.logitude} style={styles.errorText}/>
      }
     <DefaultButton title={t('app:edit')} onClick={handleSubmit} loading={loading}/>
     </View>
    )
}
